...
Citizen.CreateThread(function()
   QBCore.Functions.TriggerCallback('nuclear:getlocation', function(servercoords)
        coords = servercoords
	end)
end)
...
Citizen.CreateThread(function()
	...
	while true do
		...
		if QBCore ~= nil then
			local requiredItems = {
				[1] = {name = QBCore.Shared.Items["bluechip"]["name"], image = QBCore.Shared.Items["bluechip"]["image"]},
			}
			...
			if dist <= 1.3 then										 	
				...
				if IsControlJustPressed(0, 51) then
					QBCore.Functions.TriggerCallback('QBCore:HasItem', function(result)
						...
						QBCore.Functions.Notify("You need blue encryption chip.", "error")
						inUse = false
						end
				end, 'bluechip')
				Citizen.Wait(10700)									
	...
	Citizen.CreateThread(function()
		while enroute == true do
			...
			if disttocoord < Config.distance then
				if Config.useNotification then
					QBCore.Functions.Notify("Kill all the enemies to steal.", "error", 2500)
				end
...
function success(x,y,z,h)
	...
	local player = PlayerPedId()
	if Config.useNotification then
		QBCore.Functions.Notify("Search for the nuclear files.", "error", 2500)
	end
		...
		while not crate do 
			...
			if disttocoord <= 3 then
				...
				if IsControlJustPressed(1, 51) then
					...
					DeleteEntity(box2)
					if Config.useNotification then
						QBCore.Functions.Notify("You received nuclear files. Now get to safety.", "error", 2500)
					end
...
function decipherAnim()
	local player = PlayerPedId()
	SetEntityCoords(player, 764.2, -1905.11, 28.56, 0.0, 0.0, 0.0, false)
	SetEntityHeading(player, 193.2)
...
function spawnPed(x,y,z)
	...
	QBCore.Functions.TriggerCallback("nuclear:getCops", function(getCops)
